...
static void valid_commas(char *line, t_map *map, char *free_line)
{
    int i;
    int count;
...
static int char_to_int_rgb(char **rgb, char *free_line, t_map *map)
{
	int c[3];
...
int	parse_rgb(char *line, t_map *map, char *free_line)
{
	...
	valid_commas(line, map, free_line);
	rgb = ft_split(line, ',');
	...
	return (char_to_int_rgb(rgb,free_line,map));
