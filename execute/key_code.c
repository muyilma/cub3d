...
static void rotate_player(t_map *m, double angle)
{
    ...
    m->player.dir_x = m->player.dir_x * cos(angle) - m->player.dir_y * sin(angle);
    m->player.dir_y = old_dir_x * sin(angle) + m->player.dir_y * cos(angle);

    m->player.plane_x = m->player.plane_x * cos(angle) - m->player.plane_y * sin(angle);
    m->player.plane_y = old_plane_x * sin(angle) + m->player.plane_y * cos(angle);
}
...
int key_code(int keycode, t_map *m)
{
    ...
    if (keycode == 119 || keycode == 65362)
        player_move(m, m->player.dir_x * move_speed,m->player.dir_y * move_speed);
    if (keycode == 115 || keycode == 65364)
        player_move(m,-m->player.dir_x * move_speed,-m->player.dir_y * move_speed);
    if (keycode == 97)
        player_move(m,-m->player.plane_x * move_speed,-m->player.plane_y * move_speed);
    if (keycode == 100)
        player_move(m,m->player.plane_x * move_speed,m->player.plane_y * move_speed);
    if (keycode == 65361)
